...
Void MultiChHd_createVcapVencVdis()
{
    ...
    for(vipInstId=0; vipInstId<capturePrm.numVipInst; vipInstId++)
    {
        ...
        pCaptureInstPrm->vipInstId          = (SYSTEM_CAPTURE_INST_VIP1_PORTB-
                                              vipInstId)%SYSTEM_CAPTURE_INST_MAX;//(SYSTEM_CAPTURE_INST_VIP0_PORTA+vipInstId)%SYSTEM_CAPTURE_INST_MAX; /* Need to change based on actual HD video decoder */
   ...
   //AlgLink_ScdblkChngConfig   blkConfig[ALG_LINK_SCD_MAX_BLOCKS_IN_FRAME];
       ...
       dspAlgPrm[0].outQueParams.nextLink     = ipcBitsOutDSPId;
       dspAlgPrm[1].outQueParams.nextLink     = SYSTEM_LINK_ID_INVALID;

       for(i = 0; i< 1; i++)
       {
         dspAlgPrm[i].enableSCDAlg = TRUE;
         dspAlgPrm[i].outQueParams.nextLink     = ipcBitsOutDSPId;
