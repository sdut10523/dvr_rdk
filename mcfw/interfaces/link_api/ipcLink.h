...
typedef struct IpcBitsOutLinkHLOS_CreateParams
{
    ...
    /**< Number of buffer to allocate per channel per buffer pool 
    *   Based on the input resolution class the IpcBitsOutLink will create
    *   different pools for allocation.
    *   Four resolution classes are supported:
    *   - CIF       (Width: <= 352 x  Height: <= 288)
    *   - D1        (Width: <= 720 x  Height: <= 576)
    *   - 720       (Width: <= 1280 x Height: <= 720)
    *   - 1080      (Width: <= 1920 x Height: <= 1080)
    *  The pools are numbered by increasing resolution.
    *  For example if input channels are a mix of D1 and HD,
    *      pool0 - D1
    *      pool1 - HD
    *  The numBufPerCh parameter allows controlling the number of
    *  buffers to be allocated for each pool.
    *  Suppose there are 12D1 channels and 6HD channels and application
    *  wants to allocate 
    *  - 6 buffers _per_ channel for each D1 channel and
    *  - 4 buffers _per_ channel for each HD channel then it can set the
    *  numBufPerCh as below:
    *    numBufPerCh[0] = 6;
    *    numBufPerCh[0] = 4;
    *  This will result in allocation of total 6 (numBufPerCh) * 12 (NumChannels)
    *  D1 bitstream buffers
    *  This will result in allocation of total 4 (numBufPerCh) * 6 (NumChannels)
    *  HD bitstream buffers
    */
