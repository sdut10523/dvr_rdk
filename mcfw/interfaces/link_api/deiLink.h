...
typedef struct
{
	...
	DeiLink_OutputScaleFactor  outScaleFactor[DEI_LINK_MAX_OUT_QUE];
...
static inline void DeiLink_CreateParams_Init(DeiLink_CreateParams *pPrm)
{
    ...
    for(outId=0; outId<DEI_LINK_MAX_OUT_QUE; outId++)
    {
        ...
        pPrm->outScaleFactor[outId].ratio.widthRatio.numerator = 1;
        pPrm->outScaleFactor[outId].ratio.widthRatio.denominator = 1;
        pPrm->outScaleFactor[outId].ratio.heightRatio.numerator = 1;
        pPrm->outScaleFactor[outId].ratio.heightRatio.denominator = 1;
    ...
    pPrm->scaleMode = DEI_SCALE_MODE_RATIO;
